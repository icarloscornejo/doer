...
- **New skill `wk:replay` emulates a bug scenario directly in the app's own source, so it reproduces on device in any environment.** `wk:bugfix` reaches a verdict and `wk:protologs` verifies runtime behavior, but neither helps when the bug depends on a specific backend payload, a CMS entry, a feature flag, or an A/B bucket that simply does not exist on the dev's own build; Stage 6's "run it on device and confirm the fix behaves" was unverifiable for exactly those tickets. `wk:replay` closes the gap with two combinable techniques chosen with the dev: network response replay (a body captured in a Charles/HAR session, forced at the deserialization seam, never the transport layer, reusing the app's own configured parser instance so naming strategy and custom adapters still apply) and flag/kill-switch forcing (forced at the read site, never the definition/registry site). The injection point is engine-agnostic by design: it targets whatever function first turns the already-fetched raw response into typed models, not any specific HTTP client. Every edit is wrapped in a `// REPLAY START`/`// REPLAY END` sentinel block that preserves any line it replaces behind a `REPLAY-ORIG:` marker, which makes cleanup an exact, provable operation (`hooks/replay-restore.py`, shared verbatim by the new integrity guard and by cleanup's own fallback) instead of a text-match guess; a protologs-style additive-only check would have missed this, since replay legitimately deletes lines. Exactly two `PROTOLOG_RESPONSE - ` log lines per forced site (entry and applied), deliberately far short of protologs' full density, since protologs covers the rest of the slice when the dev asks for it. Runs only after the real fix is committed, and before `wk:protologs` when `wk:bugfix` Stage 6 invokes both. Two new `PreToolUse` hooks, `replay-temp-commit-integrity-guard.sh` and `replay-revert-conflict-guard.sh`, mirror the protolog pair's protection (restore-equality before every `[TEMP] REPLAY` commit, and an auto-abort backstop on a conflicted revert).

### Fixed

- **`wk:protologs`' own commit-integrity guard was fail-open on its only real invocation shape and had never actually fired in production.** `protolog-temp-commit-integrity-guard.sh` is a `PreToolUse` hook, so it runs BEFORE the command it guards; that command is `git add -A && git commit --no-verify -m "[TEMP] PROTOLOG ..."`, and reading only `git diff --cached` at hook time saw an empty, HEAD-equal index every single time, since `git add -A` had not executed yet. Verified empirically in a scratch repo: a deliberate forbidden-refactor violation was silently permitted under the real command shape and only caught when the file was staged by hand first, which is not how the skill invokes the guard. The guard now reads the union of the index, the working-tree diff against HEAD, and untracked files, so it is correct regardless of how or when the caller stages. This shipped undetected because there were no tests for any hook in this repo; `tests/hooks.sh` is new in this release specifically to close that gap, starting with a regression test that pins down the exact production command shape rather than a hand-staged approximation of it.

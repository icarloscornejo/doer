...
description: >-
  ...
  planned, fixed, and verified on device with /wk:replay (if the scenario needs
...
`replay` is informational only: `null` when Stage 6 never invoked `/wk:replay`, otherwise a short record of what was forced (`{"technique": "network | flags | both", "summary": "<one line>"}`). `/wk:replay` itself never reads or writes this field; it anchors on the repo's own git history (`PRE_REPLAY_SHA`), not on `bugfix.json`, precisely so it keeps working standalone with no ticket at all.
...
Both commits land before Stage 6 runs (working messages; the PR-ready message is chosen in Stage 6). They must stay separate from anything Stage 6 adds. Both `wk:replay`'s `[TEMP] REPLAY` commit and `wk:protologs`' `[TEMP] PROTOLOG` commit assume the fix underneath is already committed, so each one's diff contains only its own scaffolding and cleanup's revert is a clean no-op. An uncommitted fix gets tangled with the injected code in the same working-tree diff and has to be split apart by hand afterward.

...
**Ticket execution orchestrator for Claude Code.** Plugin version 6.3.0.
...
The toggles below live in the global preferences file at `${CLAUDE_CONFIG_DIR:-$HOME/.claude}/wk/preferences.json`. Most default off; the dev opts in to change behavior. The Stage 1 self-review defaults ON because it adds dev confidence at near-zero cost; turn it off if it adds noise on your tickets.

| Key | Default | Effect when enabled |
|---|---|---|
| `stage1_ac_self_review` | `true` | Stage 1 dispatches an `ac-reviewer` sub-agent (single round) that compares the AC draft against `intake.description`, `intake.raw_acs`, and `intake.context`. Findings use a fixed three-tier taxonomy: `affirmation` (mandatory; at least one per AC ID), `gap` (with `optional: true` for granularity preferences vs structural omissions), `blocker` (direct contradiction with the description). Blockers are promoted into Open Questions with `source: "self_review"` and a proposed resolution; the dev still answers ONE question for the whole Stage 1 contract. The orchestrator NEVER auto-applies fixes. Findings + dev decisions persist into `metadata.ac.self_review`. Failure modes (malformed output, timeout, empty findings) are non-fatal and Stage 1 always proceeds. |

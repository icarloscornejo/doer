...
| ~~WK-7~~ | ~~implement skills/load (Jira / Linear / GitHub import)~~ | SATELLITE | **Done in 6.0.0**: `/wk:load <ID>` auto-detects tracker by ID shape (Jira `PROJ-123` via `WK_JIRA_BASE_URL`, Linear via `WK_LINEAR_API_KEY`, GitHub `owner/repo#N` via `gh`). Writes `.doer/tickets/<ID>/metadata.json` with `intake.{description,raw_acs,context,tracker}` and identifying fields. AC extraction via `skills/load/lib/extract-acs.sh` heuristic. Idempotent with `--force` semantics |
| ~~WK-8~~ | ~~implement skills/advise (configurable personas)~~ | SATELLITE | **Done in 6.0.0**: `/wk:advise` runs configurable advisor personas against files, diffs, ticket plans, or ad-hoc text. Personas defined as JSON under `lib/advisor-personas/` (5 shipped: security, performance, mobile, accessibility, api). Multi-persona runs dispatch parallel Agent calls in a single tool block. `--target ticket:<ID>` writes findings to `.doer/tickets/<ID>/advisor-findings/<persona-id>.json` for downstream pipeline consumption. Forward-looking Stage 5 integration documented behind opt-in `preferences.md` key |
| ~~WK-9~~ | ~~implement skills/review (MR review with personas)~~ | SATELLITE | **Done in 6.0.0**: `/wk:review <pr-ref>` reviews external GitHub PRs (`gh`) and GitLab MRs (`glab`) using advisor personas from `lib/advisor-personas/`. Auto-detects platform by URL host or short-form sigil (`#` for GitHub, `!` for GitLab). Fetches metadata, diff, and comments (no disk write), dispatches personas in parallel Agent calls. Deterministic vote rule: any blocker = `request_changes`, any high = `comment`, else `approve`. Optional `--post` posts the report via `gh pr review` (with `--request-changes` or `--comment`, never `--approve`) or `glab mr note create` |
| ~~WK-10~~ | ~~implement skills/publish (MR + Jira transition)~~ | SATELLITE | **Done in 6.0.0**: `/wk:publish <TICKET-ID>` pushes the feature branch and creates a PR (GitHub via `gh`) or MR (GitLab via `glab`) for a completed doer ticket. Platform auto-detected from `origin` URL (cloud or self-hosted). Four pre-flight checks (workspace guard, metadata invariants incl. `last_green_sha` 40-char match, HEAD parity, remote reachability). PR/MR body composed from `metadata` (summary, ACs, plan, tests, lessons, tracker). Idempotent via `--reuse`. Optional `--transition <state>` performs Jira state change via REST API; failures recorded in `metadata.publish.jira_transition.error` without rolling back the PR. Flags: `--draft`, `--base`, `--dry-run`, `--reuse`, `--transition` |
| ~~WK-11~~ | ~~wire /wk:advise into Stage 5~~ | CORE | **Done in 6.0.0**: opt-in via `preferences.md` (`stage5_advisor_personas: ["security", ...]`, default `[]`). Stage 5 dispatches the listed advisor personas in iter 1 only (BEFORE the deterministic Pre-reviewer Checks). Findings ingested from `.doer/tickets/<ID>/advisor-findings/<persona-id>.json`: `severity: blocker` -> `metadata.code_review[].blockers` (loop continues until resolved); `high`/`medium`/`low` -> `suggestions`; `info` -> `info`. Each promoted entry carries `source: "advisor:<persona-id>"`. Personas do NOT re-run in iter 2/3; advisor blockers flow through the standard `prior_blockers_resolved` / `still_open` machinery. `metadata.code_review[iter=1].advisor_personas_ran` lists which personas ran. Reviewer LLM acknowledges advisor findings inline but does not re-judge them |
